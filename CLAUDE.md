...
# KioskOS - Electron & TypeScript Guidelines

> **MANDATORY:** NEVER USE EMOJIS/EMOTES.

## Core Principles
- **Type Safety Strictness**: Use TypeScript for everything. `any` is strictly forbidden. Use `unknown` combined with runtime validation (e.g., Zod) at IPC boundaries.
- **Process Isolation**: The architecture follows strict separation. [cite_start]The Renderer process is a "Dumb Client". It MUST NOT access Node.js APIs directly. All system, hardware, and database access goes through the Main process via the Context Bridge (Preload).
- **Offline-First Resilience**: Assume the network is dead. The UI must never block waiting for an external HTTP request. [cite_start]All states, including active sessions and pricing rules, must be read from and persisted to the local SQLite database  first. [cite_start]Sincronização Passiva [cite: 86] happens in the background.

## Architecture & Structure
- `src/main/`: Core Electron process. Handles the SQLite database, Auth/Token logic, Auto-updater, Background sync, and Hardware integration (e.g., agnostic printing).
- `src/preload/`: Context Bridge definitions. Strictly typed APIs exposed to the frontend (e.g., `window.api.db.getSessions()`).
- [cite_start]`src/renderer/`: React + Vite frontend. Pure UI, driven by state. No business logic regarding pricing or sync should live here.
- `src/shared/`: Shared TypeScript interfaces, types, IPC channel constants, and DTO definitions used by both Main and Renderer.

## IPC (Inter-Process Communication) Standards
- **Naming Conventions**: Channels must be namespaced using `domain:action` (e.g., `sync:push-sessions`, `db:calculate-tariff`, `hw:print-receipt`).
- **Typing**: Both `ipcMain.handle` and `ipcRenderer.invoke` MUST use shared TS interfaces for parameters and return types to ensure end-to-end type safety.
- **Error Boundaries**: IPC handlers must catch internal errors and return serialized, typed error objects. Do not throw raw Node errors across the IPC bridge, as they lose their prototype and can leak system paths.

## Database & State (Main Process)
- [cite_start]**SQLite is King**: The local SQLite database is the mirror of the business rules and the source of truth for the local application.
- **Migrations**: Keep SQLite schema definitions version-controlled. Migrations must run automatically and safely on Main process startup before the window loads.

## Testing
- Use **Vitest** for Main process logic, utilities, and Shared logic.
- Use **Playwright** for E2E testing of the Electron windows and IPC flows.
- Write exact, behavioral descriptions: `it('should calculate local fractional pricing when offline', ...)`
...
<!-- @aipim-signature: d3825403901c137a549e30a97b76751a6ee1fa81a8b40b6baba7e0e7adc72041 -->
<!-- @aipim-version: 2.0.0-alpha -->
